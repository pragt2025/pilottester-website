...
const testimonials = [
  ...
  },
  {
    id: 'testimonial-4',
    name: "Michael Chen",
    title: "Founder, Appify",
    quote: "The 14-day continuous testing requirement was a major hurdle for us. This platform made it effortless to find real, engaged testers.",
  },
  {
    id: 'testimonial-5',
    name: "Emily Rodriguez",
    title: "Product Manager",
    quote: "An essential tool for any developer looking to meet Google Play's closed testing requirements. The quality of testers is top-notch.",
  },
  {
    id: 'testimonial-6',
    name: "David Lee",
    title: "Android Engineer",
    quote: "I was able to get my app to production in record time thanks to this community. The test-to-earn model is brilliant.",
  },
];
...
const TestimonialCard = ({ testimonial }: { testimonial: typeof testimonials[0] }) => {
    ...
    return (
        <Card className="bg-card shadow-lg rounded-lg flex-shrink-0 w-[350px]">
            <CardContent className="p-6">
                ...
                <blockquote className="text-muted-foreground mb-6 h-20">"{testimonial.quote}"</blockquote>
...
export function Testimonials() {
  return (
    <section id="testimonials" className="py-12 md:py-24 lg:py-32 bg-background relative overflow-hidden">
        <style jsx>{`
            @keyframes marquee {
        ...
        `}</style>
      <div className="absolute top-0 right-0 w-1/2 h-full bg-no-repeat bg-right-top dark:opacity-10" style={{ backgroundImage: "url('data:image/svg+xml,%3Csvg width=\"719\" height=\"778\" viewBox=\"0 0 719 778\" fill=\"none\" xmlns=\"http://www.w3.org/2000/svg\"%3E%3Cpath d=\"M-112.5 778C89.5 700.5 457.5 678.5 613 429.5C768.5 180.5 526 -134.5 -112.5 -134.5V778Z\" fill=\"%23F3F4FF\"/%3E%3C/svg%3E')", zIndex: 0 }}></div>
      ...
      <div className="relative flex mt-12 group">
        <div className="flex animate-marquee">
            {[...testimonials, ...testimonials].map((testimonial, index) => (
                <div key={`marquee-1-${index}`} className="mx-4">
        ...
        <div className="flex animate-marquee" aria-hidden="true">
            {[...testimonials, ...testimonials].map((testimonial, index) => (
                 <div key={`marquee-2-${index}`} className="mx-4">
                    <TestimonialCard testimonial={testimonial} />
                </div>
            ))}
        </div>
